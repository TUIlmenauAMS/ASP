#This is a library that implements an MDST filter bank (sine modulation), including a real time implelentation.
#Obtained from the QMF program by using an MDCT window instead the qmf window.
#The "main" is an example which reads in an audio .wav file, block-wise, computes the QMF subbands of the 
#analysis filter bank, then writes them to a python "pickle" file.
#As such, it can read and process arbitrary long audio files, without memory problems!
#It has 2 arguments, the input wav file, and the output file, which contains the subband values after 
#the analysis filter bank
#example usage: 
#             python MDST_realtime_analysis.py mixed.wav mixed_mdst_y.bin
...
def analysisqmf_sinmod_realtime(xrt,Fa,N):
	#computes the QMF subband samples for each real time input block xrt. 
	#Conducts an implicit polynomial multiplication with folding matrix Fa of 
	#the polyphase matrix of the QMF filter bank, using
	#internal memory of the past input blocks.
...
if __name__ == '__main__':
	...
	#open pickle file to write the subband samples:
	wfw=open(sys.argv[2], 'w')
	...
	for m in range(length/N):
	   ...
	   y=analysisqmf_sinmod_realtime(x,Fa,N)
	   plt.plot(y)
	   plt.show(block=False)
	   pickle.dump(y,wfw)
