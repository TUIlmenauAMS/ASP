#This is a library that implements a synthesis MDST filter bank (sine modulated), including a real- time implelentation.
#Obtained from the QMF program by using an MDCT window instead the qmf window.
#The "main" is an example, which reads in an python pickle file which contains the subband samples, block-wise, 
#then computes the synthesis QMF filter bank to reconstruct the audio signal from the subbands, and writes them, block wise, into another audio .wav file. 
#As such, it can read and process arbitrary long audio files, without memory problems!
#example call from a terminal window: 
#        python MDST_realtime_synthesis.py mixed_mdst_y.bin mixed_rek_sin.wav
#It loads the qmf coefficient file 'MDCTsinwin1024bands.mat'
# 
...
def synthesisqmf_sinmod_realtime(y,Fs,N):
	#computes the inverse QMF for each subband block y, 
	#conducts an implicit polynomial multiplication with synthesis folding matrix Fs 
	#of the synthesis polyphase matrix of the QMF filter bank, using
	#internal memory for future output blocks.
...
if __name__ == '__main__':
	...
	#Synthesis Folding matrix: 
	#Fs=hs2Fs3d_fast(qmfwin,N)
	Fs=hs2Fs3d_sinmod_fast(qmfwin,N)
	...
	wfw = wave.open(sys.argv[2], 'wb')
   	wfw.setnchannels(nchan)
   	wfw.setsampwidth(bytes)
	...
	while True:
	   ...
	   xrek=synthesisqmf_sinmod_realtime(y,Fs,N)
